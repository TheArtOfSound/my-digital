...
Deployed on Cloudflare: **https://my-digital.bryanleonard237.workers.dev** (Worker + D1 + static SPA, real QEV Vault V2 crypto on the edge via the `@noble` backend). Custom domain `mydigital.imagineqira.com` pending an apex nameserver move to Cloudflare — see [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md). Payments are in `mock` mode until live Stripe keys are configured.
